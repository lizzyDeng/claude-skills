...
Phase 2+3: /goal 自主执行（Plan 确认后自动触发）
  ...
  3.2  验证意图         [/goal + hook auto-detect] verification-plan.json：把锁定 AC 编成正常用户路径(journeys)
  3.3  验证执行         [/goal + hook auto-detect] 看一眼再点：按用户路径走一遍，每步截图→有序旅程时间线
  3.4  AC 裁判+结构 gate [/goal + hook auto-detect] 对抗裁判逐 AC 出证→确定性结构 gate→旅程测试报告(HTML)
...
## Phase 3：AC 驱动【用户旅程】验证（替代「数 turn」E2E）

验证单元是**一条正常用户路径(journey)，不是孤立 AC，更不是 turn**。像真用户那样把
feature 从头走到尾、**每一步截图**，AC 是沿途检查点。头号交付是**人读测试报告**：有序点击
路径 + 每步截图 + 该步证了哪条 AC。

- **3.2 验证意图 · 旅程脚本**：从锁定 AC + 实现 diff 派生 `verification-plan.json`，把 AC 编成
  `journeys[]`。每条 journey = `{id, title, differential?, steps[]}`；每个 step =
  `{no, surface, action(open/click/type/observe/toggle), intent, expect, proves_acs[], hints}`。
  所有步骤 `proves_acs` 并集必须覆盖每条 P0/P1 AC（结构 gate 复核覆盖，缺/漂移 FAIL）。
- **3.3 验证执行 · 按用户路径走一遍，每步截图**：浏览器 agent（默认 `agent-browser`）逐 journey 顺序
  走「看一眼再点」（读真实 a11y 树 → 看到真实元素再点 → **每走一步就截图** → 采 network/DOM）→ 写成
  有序 `<journey_id>.journey.json`（步骤含 `screenshot`/`caption`/`state`/`proves_acs`）+
  `evidence-manifest.json`（每张截图 sha256）。**UI 步必有截图**；API 步可用 network/响应替代。
  cross-端旅程跨 admin/user/api；differential 旅程走 ON（每步截图）→ OFF（每步截图）→ teardown 两态。
- **3.4 AC 裁判 + 结构 gate + 测试报告**：① 独立**对抗裁判**（≠实现者，真看截图）逐 AC 出
  `verify-judge.json`（verdict + 引用**证该 AC 那一步**的真实截图 + reason）；② `verify_gate.py` 跑
  **确定性结构 gate**（六道：AC 覆盖/必经表面真实走过/differential/证据真实(sha256)/裁判引用有效/派生终判）；
  ③ 永远生成 `verify-report.html`（旅程时间线：点击路径 + 每步截图 + 逐 AC 裁判）。
  exit 0=PASS / 3=SURFACE(证据弱，看报告后 `verify_confirm` 放行) / 1=FAIL。

cross-端覆盖**靠结构强制**：diff 改过的 surface 必须被旅程**真实走过**（改哪个 app_paths 就必须走哪个表面），
`differential` 要求 ON/OFF 两态对照（只证共存不证因果 = FAIL）。`min_turns` 已废，不参与任何判定。
UI feature 不再需要 N/A 死墙。
...
验证证据 manifest（`evidence-manifest.json`）的 sha256 记入 gate state（`verify_evidence_hash`，与 `e2e_result_hash` 同源），每张截图 artifact 也按 manifest 记 sha256。manifest 或截图在执行后被改 → Step 3.3/3.4 结构 gate 必须 `FAIL`（证据真实检查）。裁判 `verify-judge.json` 每条 verdict 必须引用真实存在、**属于证该 AC 那一步**的截图，伪造引用 / 借别的 AC 步骤的图 = 结构 gate FAIL（反橡皮图章）。
