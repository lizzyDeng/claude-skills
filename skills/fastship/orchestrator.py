...
🔴🌳 计划树结构（让执行层永不持有全 plan）—— plan 正文用 HTML 注释锚点切成 root + 节点：
  - 共享层（设计决策 / AC→E2E 映射 / File Structure / 图示 等所有节点都要看的内容）放在
    plan 开头第一个节点锚点之前；可选用 `<!-- fastship:root -->` … `<!-- fastship:/root -->` 显式包住。
  - 每个实现任务写成一段，段首紧贴一行锚点 `<!-- fastship:node <id> -->`（<id> 匹配 ^[a-z0-9][a-z0-9_-]*$），
    其下接 `### <标题>` 与该任务【完整丰富度】的实现正文（逐字写够，不缩水）。
  - 节点自包含纪律：节点正文只引用 root 里的符号，禁「见 Task 2 / 如前述」式跨节点引用
    （机械切片后会变悬空）；任务之间的数据传递走下面 nodes[].inputs/outputs 声明，不靠正文 prose。

🔴 plan 末尾必须嵌【唯一】一个机器可验证的契约块，紧贴 `<!-- fastship:contract -->` 标记 + ```json
  围栏，同时含 nodes + ac_mapping + exclusive_forks 三个 key（全文只许一个此标记，禁多块）：
  <!-- fastship:contract -->
  ```json
  {"nodes": [
    {"id": "task-1", "title": "<简述>", "deps": [],
     "inputs": ["root:<root 层声明的符号>"], "outputs": ["sym:<本节点对外产出符号>"],
     "files": ["repo-relative/具体文件.rs"]}
  ],
   "ac_mapping": [
    {"ac_id": "<1A P0 AC 的 id>", "tasks": ["task-1", ...], "e2e": ["E2E scenario1", ...]}
  ],
   "exclusive_forks": [
    {"id": "tf-1", "decision": "<技术选型 either/or>", "status": "open"}
  ]}
  ```
  · nodes[]：每个 id 对应正文一个 `<!-- fastship:node <id> -->` 锚点（一一对应，缺一即 FAIL）；
    deps 引用其它 node id（构成无环 DAG）；outputs 全局唯一；inputs 每项是 `root:<符号>` 或某【上游】node 的 output；
    files 只许 repo-relative 具体文件路径（禁 glob/目录/绝对路径/..）；共享同一文件的两节点必须有依赖边相连。
  · ac_id 引用 1A 真实 AC id（P0+P1，禁 dangling/编造）；每条 AC 至少 1 task + 1 E2E；不许重复 ac_id；
    tasks 引用 nodes[].id（禁自由文本）；每个 node 要么被某 ac_mapping.tasks 引用、要么显式 supporting_for。
  · exclusive_forks 选填：总结者拿不定的【技术选型 either/or】摊成 open fork（禁拍板），交 1.5 grill 人裁；
    无技术岔路就留空/省略 → 1.5 grill 自动跳过（F4，codex 1.5c 照跑）。

🔴 引擎硬验证(validate_plan, non-bugfix)：1A 每条 P0/P1 AC 必须在 ac_mapping 中映射到 task(node id)+E2E，
   且 nodes[] 必须通过 node graph 校验（结构/id 正则/outputs 唯一/无环/无悬空/文件重叠须有依赖边），
   通过后引擎自动把 plan 拆成计划树（root.md / nodes/<id>.md / briefs/<id>.md / skeleton.json）；
   缺映射/引用不存在 AC/未全覆盖/node graph 违例/锚点与 nodes 不一致 = 当场 FAIL（不等 codex）。
   bugfix 无 1A，只验 writing-plans 签名（不拆树）。
...
/goal 模式下用 dynamic workflow（ultracode）执行计划树 —— 🌳【driver 只握 skeleton，不读 plan 正文】：
...
🔴 模型档位（Phase 2 = 推理档）：你为本阶段编写的 dynamic workflow 里，【每一个 implement
  ...
  2. 🌳 读【计划树骨架】决定扇出（不读全 plan）：从 `goal` 命令输出的 skeleton_path（=
       artifacts.plan_tree.skeleton_path）读 skeleton.json，拿到 nodes[] 的 id/deps/files。
       - 按 deps 做拓扑序；deps 不相连且 files 不相交的 node → 可并行组；有依赖边或改同一批文件 → 串行。
       - 不相交组【数量 ≥2】才用 Workflow parallel() 并行实现；只有一条链时直接 subagent-driven 串行。
       - 🔴 每个 implement subagent【只收一条 briefs/<id>.md 路径】（= root + 本 node 正文 + 上游 output 契约），
         由 subagent 自己读这一片；driver【绝不把 node 正文读进自己 context】——否则逐片累积又回到满 plan，
         本阶段去单体化的全部意义就没了。driver 手里只有 skeleton（id/deps/files/status），无任何 node 正文。
       - 并行 agent 在【同一 worktree】内只编辑各自 node.files 内的文件，【不各自 commit】
  ...
  3. implement→review pipeline：每个 node 实现完立刻被对抗性 review
  ...
  6. 每个 node 完成后运行 status，让 /goal 评估器看到 [FASTSHIP_GOAL] 的 nodes_done/nodes_total 进度。
