...
def validate_plan(orch: dict, hook: dict) -> tuple:
    ...
    # 1B 技术方案: for features the plan must map every locked 1A P0 AC to
...
def _check_requirements_contract(gate: dict) -> tuple:
    ...
    for item in gate["p0"]:
        ...
        # the same dup-id bypass the concern check above closes.
        for a in acs:
            if not isinstance(a, dict):
                return False, f"p0 {pid} 的 observable_ac 项必须是 {{id, assertion}} object"
            aid = a.get("id")
            if not isinstance(aid, str) or not aid.strip():
                return False, f"p0 {pid} 的 observable_ac 项缺少非空 id"
            if not isinstance(a.get("assertion"), str) or not a["assertion"].strip():
                return False, f"p0 {pid} 的 AC {aid} 缺少非空 assertion"
            if aid in ac_ids:
                return False, f"observable_ac id 重复: {aid} — AC id 须全局唯一,否则 1B 覆盖比对会漏掉缺口"
            ac_ids.add(aid)
...
def _collect_p0_ac_ids(requirements_gate: dict) -> set:
    """Every P0 AC id declared in the 1A requirements contract — the universe 1B
    must cover. The gate has already passed _check_requirements_contract (AC ids
    present, non-empty, globally unique); stays defensive about shape regardless."""
    ids = set()
    for item in (requirements_gate.get("p0") or []):
        if not isinstance(item, dict):
            continue
...
def _check_plan_mapping(p0_ac_ids: set, plan_gate: dict) -> tuple:
    """Pure 1B discipline check: every 1A P0 AC must map to ≥1 task AND ≥1 E2E
    scenario, by reference to a real AC id. No I/O. FAIL on:
      ...
      - uncovered: a P0 AC with no mapping entry at all — the core 「缺映射当场 FAIL」.
    """
    if not isinstance(plan_gate, dict):
        return False, "plan 映射契约必须是 JSON object"
    mapping = plan_gate.get(PLAN_MAPPING_FIELD)
    if not isinstance(mapping, list) or not mapping:
        return False, f"plan 缺少非空 {PLAN_MAPPING_FIELD} 数组 — 每条 1A P0 AC 须映射到 task+E2E"
    mapped = set()
    for m in mapping:
        ...
        mapped.add(aid)
    uncovered = sorted(p0_ac_ids - mapped)
    if uncovered:
        return False, ("1A P0 AC 未在技术方案中映射(缺 task+E2E 映射,1B FAIL): "
                       + ", ".join(uncovered))
    return True, f"AC→task+E2E 映射完整 ({len(p0_ac_ids)} 条 P0 AC 全覆盖)"
...
def _validate_plan_ac_mapping(orch: dict, plan_content: str) -> tuple:
    ...
    p0_ac_ids = _collect_p0_ac_ids(req_gate)
    if not p0_ac_ids:
        return False, "1A 需求定稿无 P0 AC id,无法做 1B 映射校验"
    ...
    if plan_gate is None:
        return False, (f"plan 缺少机器可验证的 {PLAN_MAPPING_FIELD} JSON 契约块 — "
                       "1B 须为每条 1A P0 AC 显式映射 task+E2E")
    ok, msg = _check_plan_mapping(p0_ac_ids, plan_gate)
    if not ok:
        return False, msg
...
派 Workflow 多角色技术法庭（RD/QA/总结者，吃 1A 需求定稿 .fastship-requirements.md 的 P0 AC 作输入）：
  - RD：每条 P0 AC 的实现路径（涉及文件/模块 + 任务拆分）
  - QA：每条 P0 AC 的 E2E 验证 scenario（可观察断言，禁 button visible/page loads 弱 case）
  - 总结者：合成技术方案，并为【每一条 1A P0 AC】显式映射 task + E2E（书记员只搬运不漏）
...
🔴 plan 末尾必须嵌一个机器可验证的 ```json 映射契约块（key = ac_mapping）：
  ...
  ```
  ac_id 必须引用 1A 真实 AC id（禁 dangling/编造）；每条 AC 至少 1 task + 1 E2E；不许重复 ac_id。
  exclusive_forks 选填：总结者拿不定的【技术选型 either/or】摊成 open fork（禁拍板），交 1.5 grill 人裁；
  无技术岔路就留空/省略 → 1.5 grill 自动跳过（F4，codex 1.5c 照跑）。

🔴 引擎硬验证(validate_plan, non-bugfix)：1A 每条 P0 AC 必须在 ac_mapping 中映射到 task+E2E，
...
🔴 GATE: FAIL → 先更新 plan 修复 findings，再重新调 codex review。
   orchestrator 检测到 FAIL 自动回退到 1.4（1B 技术方案），更新后重走 1.5 → 1.5c。
🔴 GATE: PASS → 自动推进到用户确认。"""),
    ...
    if detected:
        ...
        if detected == "1.5c":
            ...
            if not ok and "FAIL" in msg:
                ...
                # F4: the rewritten plan re-derives its fork state on 1.4 re-validate,
                # so drop the stale open-fork signal and any prior auto-skip of 1.5 —
                # whether the grill runs this round is decided fresh from the new plan.
                orch.setdefault("artifacts", {}).pop("plan_open_fork_ids", None)
