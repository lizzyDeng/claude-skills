...
下面 JSON 里的 `<PROJECT_ROOT>` 必须替换为 `git rev-parse --show-toplevel` 的绝对路径。
不要使用相对 `.claude/tools/...`，否则 agent `cd` 到子目录或其他目录后 hook 会找不到脚本。
...
# 多需求并行：列出/切换独立 session
.claude/tools/fastship list
.claude/tools/fastship use <session-id>

# 指定 session 启动（Forge feature 推荐使用 feature slug）
.claude/tools/fastship start --session <feature-slug> "<需求>"

# 重置（新需求开始时）
.claude/tools/fastship reset
...
python3 "$(git rev-parse --show-toplevel)/.claude/hooks/ship_verify_gate.py" knowledge_skip \
  --reason "纯文档改动，未触及代码或行为"

# 记录本轮 E2E loop 结果（每轮 E2E 跑完都要调一次）
# Sequential：
python3 "$(git rev-parse --show-toplevel)/.claude/hooks/ship_verify_gate.py" loop_record --outcome pass
python3 "$(git rev-parse --show-toplevel)/.claude/hooks/ship_verify_gate.py" loop_record --outcome fail \
