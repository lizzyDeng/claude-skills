...
## 0. 前置依赖：superpowers 插件（🔴 必需）

fastship 的阶段 1.5 / 阶段 2 强制调用 superpowers 里的 skill：

- `writing-plans` — 阶段 1.5 写计划
...
输出应包含 `Plan / Test / E2E` 三行状态。

### Gate 一览

| Gate | 触发 | 作用 |
|------|------|------|
| **Plan Gate** (pre_edit) | 编辑代码文件 | 未写 plan 文件 → BLOCK，提示调 `writing-plans` |
