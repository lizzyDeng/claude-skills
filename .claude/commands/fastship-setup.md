...
## Step 2: 安装 fastship（source-linked，默认）

使用 source-linked 安装，避免把 fastship 源码复制到业务工程。后续 fastship 改动只需要修改 `/Users/apple/works/claude-skills`，业务工程里的稳定入口会通过 symlink 自动指向最新源文件。

运行：

```bash
python3 /Users/apple/works/claude-skills/skills/fastship/scripts/install_source_link.py \
  --project "$(git rev-parse --show-toplevel)" \
  --replace
```

如果当前项目也使用 `/forge`（例如 aifriends），加 `--with-forge`：
...
安装结果应是 symlink，而不是复制文件：

```bash
ls -l .claude/commands/fastship.md \
      .claude/hooks/ship_verify_gate.py \
      .claude/tools/fastship \
      .claude/tools/fastship_orchestrator.py \
      .claude/tools/fastship_state.py
```

这些路径必须指向 `/Users/apple/works/claude-skills/skills/fastship/...`。

## Step 3: 检查 hooks 配置

`install_source_link.py` 会合并 `.claude/settings.local.json`，并保留已有 permissions/其他配置。hooks 仍调用当前项目里的稳定路径：

- `.claude/tools/fastship_orchestrator.py pre_edit`
- `.claude/tools/fastship_orchestrator.py pre_bash`
- `.claude/tools/fastship_orchestrator.py post_edit`
- `.claude/tools/fastship_orchestrator.py post_bash`

这些文件是 symlink，实际执行的是 `claude-skills` 源树。

## Step 4: 安装 E2E 工具（项目模板，可复制）
...
## Step 5: 配置 hooks（仅当 Step 2 未自动合并时手动做）
...
1. 检测到的技术栈
2. 安装了哪些 symlink（包括 `.claude/commands/fastship.md` skill 定义）
...
4. **Orchestrator 是 hook 入口**：每次 Edit/Write/Bash 自动触发项目本地 symlink，实际执行 `claude-skills` 源树 orchestrator，orchestrator 内部委托源树 ship_verify_gate
