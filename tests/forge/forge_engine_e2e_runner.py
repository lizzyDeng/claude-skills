...
Every turn records an explicit `expect` and the observed `actual` — no claim is
self-certifying. Result schema is compatible with tests/e2e_gate.py
(scenarios -> rounds -> turns). Exit 0 iff every turn held.
...
def run():
    ...
    def turn(action, expect, fn):
        """fn() -> (ok, actual). Record explicit expect + observed actual."""
        t0 = time.time()
        try:
            ok, actual = fn()
    ...
    try:
        ...
        all_ok &= turn("audit: --strict flips on plan-without-metric", "non-strict exit 0, strict exit 1",
        ...
        all_ok &= turn("compaction: read from linked worktree", "epoch == MAIN log 12:00:00Z",
                       lambda: (forge_gate._last_compaction_epoch() == _epoch("2026-06-03T12:00:00Z"),
                                f"epoch={forge_gate._last_compaction_epoch()} want={_epoch('2026-06-03T12:00:00Z')}"))
        _write_log(wt, "2026-06-03T08:00:00Z")  # local older than main 12:00
        all_ok &= turn("compaction: max(shared, local)", "epoch == newer MAIN 12:00:00Z (not local 08:00)",
                       lambda: (forge_gate._last_compaction_epoch() == _epoch("2026-06-03T12:00:00Z"),
                                f"epoch={forge_gate._last_compaction_epoch()}"))
        forge_gate.get_repo_root = lambda: c
        _write_log(c, "2026-06-03T09:30:00Z")
        all_ok &= turn("compaction: main worktree unchanged", "epoch == local 09:30:00Z (paths coincide)",
                       lambda: (forge_gate._last_compaction_epoch() == _epoch("2026-06-03T09:30:00Z"),
                                f"epoch={forge_gate._last_compaction_epoch()}"))
    finally:
